...
export const RECOMMENDED_MODELS: Record<AIFunctionId, { provider: AIProviderId; model: string }> = {
  ...
  translation: { provider: 'anthropic', model: 'claude-opus-5-5' }, // $4/$20

  // 吃模型腦內的梵藏佛學知識，小模型會很有自信地編造詞源。用量極小。
  dictionary_lookup: { provider: 'anthropic', model: 'claude-opus-5-5' }, // $4/$20
  ...
  term_extraction: { provider: 'openai', model: 'gpt-6-luna' }, // $0.10/$0.50

  // 機械重排，整篇進整篇出 → 輸出價是成本主體。
  formatting: { provider: 'openai', model: 'gpt-6-luna' },
  source_formatting: { provider: 'openai', model: 'gpt-6-luna' },
  translation_formatting: { provider: 'openai', model: 'gpt-6-luna' },

  // 原始 HTML 又髒又長，需要一點理解力，不用最陽春的。
  url_cleanup: { provider: 'openai', model: 'gpt-6-luna' },

  // 批次跑幾千筆、每筆只輸出兩三個字。與最低單價的 gpt-5-nano 差距可忽略，
  // 統一用同一個新世代輔助模型比較好維護。
  glossary_fill: { provider: 'openai', model: 'gpt-6-luna' }, // $0.10/$0.50
}
