...
關鍵格式規則：
1. 嚴禁添加原文沒有的字詞。所有欄位內容只能來自原文，不得自行補上音韻注記、補字、潤飾、改寫。
2. 中文音譯欄位的音韻注記（二合、三合、二合引、引）處理方式：
   - 僅當原文中**確實出現**這些字時，才用半形括號 () 包起來。
   - 原文有「播引跋姿普吒二合」 → 輸出 "播(引)跋姿普吒(二合)"
   - 原文有「嚩日囉二合引野」 → 輸出 "嚩日囉(二合引)野"
   - 原文有「娑嚩二合引訶」 → 輸出 "娑嚩(二合引)訶"
   - 原文是「唵」 → 輸出 "唵"（不要補成「唵(二合)」或任何別的形式）
   - 原文是「薩嚩」 → 輸出 "薩嚩"（不要補注記）
   - 原文是「娜訶曩」 → 輸出 "娜訶曩"（不要補注記）
3. 其他 row（悉曇、IAST、字義 等）不要加任何額外括號。
4. 缺少對應內容的 segment 欄位填空字串 ""，不要憑空產生內容。
5. 不要把猜測當事實。不確定的內容一律用空字串。
...
最高原則：**嚴禁添加使用者輸入沒有的字詞。** 不得自行補注記、補字、改寫、潤飾。

請：
1. 修正欄位對齊（把錯位的欄位歸位，但不能改動內容本身）。
2. 補上明顯遺漏的內容只限於：使用者輸入有提到、但被誤放到別欄的字串；其他一律維持空字串。
3. 確認 summary 與 segments 邏輯一致；若 summary 缺失，可根據 segments 已有的字義產出一句通順繁體中文意譯。
4. 中文音譯欄位的括號處理：
   - 若內容**已包含「二合 / 三合 / 二合引 / 引」這些字**且未用括號 → 補上括號。例如「播引跋姿普吒二合」改為「播(引)跋姿普吒(二合)」。
   - 若內容**沒有**這些字 → 維持原樣，**絕對不要**補加注記。例如「唵」維持「唵」、「薩嚩」維持「薩嚩」。
   - 已有括號 → 不變動。
5. 不要擅自更動使用者明確填寫且看起來合理的內容。
6. 維持原本的 rows 順序與 label。
