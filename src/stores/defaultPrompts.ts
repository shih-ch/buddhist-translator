...
關鍵格式規則：
1. 「中文音譯」這個 row 中，音韻注記（二合、三合、二合引、引等）必須用半形括號 () 包起來：
   - 「播引跋姿普吒二合」 → 輸出 "播(引)跋姿普吒(二合)"
   - 「嚩日囉二合引野」 → 輸出 "嚩日囉(二合引)野"
   - 「娑嚩二合引訶」 → 輸出 "娑嚩(二合引)訶"
2. 其他 row 不要加額外括號。
3. 缺少對應內容的 segment 欄位填空字串 ""。
4. 不要把猜測當事實。不確定的內容用空字串。
...
請：
1. 修正欄位對齊。
2. 補上明顯遺漏的內容；無法確定的維持空字串。不要憑空捏造。
3. 確認 summary 與 segments 邏輯一致；若 summary 缺失，根據 segments 的字義產出一句通順繁體中文意譯。
4. 中文音譯欄位中的「二合、三合、二合引、引」等音韻注記，補上半形括號 ()。例如「播引跋姿普吒二合」改為「播(引)跋姿普吒(二合)」。已有括號的不變動。
