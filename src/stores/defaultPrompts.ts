...
請：
1. 修正欄位對齊（把錯位的欄位歸位，但不能改動內容本身）。
2. 補空白的「字義」欄位：用佛教標準漢譯填入（依據該行的 IAST 或音譯）。完全無法判斷才留空字串。
3. 補空白的「中文音譯／悉曇／IAST」欄位：只能補使用者輸入有提到、但被誤放到別欄的字串。沒提到就留空。
4. 確認 summary 與 segments 邏輯一致；若 summary 缺失，根據 segments 字義產出一句通順繁體中文意譯。較長時分多行（每句一行用 \\n 分隔）。
5. 中文音譯欄位的括號處理：
   - 若內容**已包含「二合 / 三合 / 二合引 / 引」這些字**且未用括號 → 補上括號（例：「播引跋姿普吒二合」→「播(引)跋姿普吒(二合)」）
   - 若內容**沒有**這些字 → 維持原樣，**絕不**補加注記（例：「唵」維持「唵」）
   - 已有括號 → 不變動
6. 不要擅自更動使用者明確填寫且看起來合理的內容。
7. 維持原本的 rows 順序與 label。
