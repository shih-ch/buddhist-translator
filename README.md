...
>
> 💡 **只想準備一把金鑰**：預設配置用到 Anthropic（翻譯、辭典、真言、OCR）與 OpenAI（排版、術語）兩家。
> 只填 Anthropic 也能跑完整流程，把輔助功能一併改成 Claude Sonnet 5 或 Haiku 4.5 即可；
> 但 **OCR 圖片辨識只能用 Anthropic 模型**，這項無法用其他家取代。
> 各功能該挑哪個模型見 [建議配置](#建議配置)。
...
這份建議內建於應用：**設定 → AI 功能 →「套用建議配置」** 可一鍵套用（只改模型，不動 prompt）。

| 功能 | 建議模型 | 價格（輸入/輸出） | 為什麼 |
|------|----------|------------------:|--------|
| **翻譯** | Claude Opus 5.5 | $4 / $20 | 長文、多語種、五欄對照的嚴格格式，錯了要手動修——唯一值得花錢的地方 |
| **格式整理** | GPT-6 Luna | $0.1 / $0.5 | 機械重排，整篇進整篇出 → 輸出價才是成本主體 |
| **術語提取** | GPT-6 Luna | $0.1 / $0.5 | 要吐合法 JSON，比純排版更吃指令遵循；輸出量小，貴一點無所謂 |
| **網頁摘取整理** | GPT-6 Luna | $0.1 / $0.5 | 原始 HTML 又髒又長，需要一點理解力，不用最陽春的 |
| **原文排版** | GPT-6 Luna | $0.1 / $0.5 | 機械重排，整篇進整篇出 → 輸出價才是成本主體 |
| **譯文排版** | GPT-6 Luna | $0.1 / $0.5 | 機械重排，整篇進整篇出 → 輸出價才是成本主體 |
| **佛學辭典** | Claude Opus 5.5 | $4 / $20 | 吃模型腦內的梵藏佛學知識，小模型會很有自信地編造詞源；用量極小 |
| **術語翻譯補齊** | GPT-6 Luna | $0.1 / $0.5 | 批次跑幾千筆、每筆只輸出兩三個字；與更低單價的差距可忽略，統一用同一個輔助模型 |
| **真言抽取** | Claude Sonnet 5 | $2 / $10 | 咒語逐段對齊很挑精度，省這裡會反過來花你時間 |
| **真言重排** | Claude Sonnet 5 | $2 / $10 | 同真言抽取，格式精度優先 |
| **OCR 圖片辨識** | Claude Sonnet 5 | $2 / $10 | 高解析視覺（2576px）對藏文與天城體差很多。**此功能只能用 Anthropic 模型** |

> **成本重點**：只有「翻譯」會左右帳單。其餘十項加起來不到一篇文章成本的 10%，
> 所以輔助功能的挑選標準是「不要出錯害我重跑」，而不是「最便宜」。

以一篇原文 6,000 tokens、譯文 10,000 tokens 的文章估算翻譯成本：

| 翻譯模型 | 每篇 |
|----------|-----:|
| GPT-6 Astra | $0.56 |
| Claude Fable 5.1 | $0.56 |
| Claude Opus 5.5 ⭐ | $0.22 |
| GPT-6 Sol | $0.11 |
| Claude Sonnet 5 | $0.11 |

### 支援的 AI 模型與價格

所有價格為 **美元 / 每百萬 tokens**，⭐ 為上表的建議選擇。即時費用追蹤內建於應用中。
...
| Claude Fable 5 (legacy) | $10 | $50 |
| Claude Opus 5 (legacy) | $5 | $25 |
| Claude Opus 4.8 (legacy) | $5 | $25 |
| Claude Opus 4.7 (legacy) | $5 | $25 |
| Claude Sonnet 4.6 (legacy) | $3 | $15 |
| Claude Opus 4.6 (legacy) | $5 | $25 |
| Claude Opus 4.5 (legacy) | $5 | $25 |
| Claude Sonnet 4.5 (legacy) | $3 | $15 |
...
> 部分價格為限時促銷：GPT-5.6 Sol 的 $4/$20 至少維持到 2026-11-21；
> Gemini 3.6–3.8 Flash 的 $0.75/$3.75 於 2027-01-01 起回到 $1.50/$7.50。
> Perplexity 模型整合即時網路搜尋，另有每次搜尋的額外費用。
